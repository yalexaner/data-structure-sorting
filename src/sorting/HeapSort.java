...
public class HeapSort implements BaseSort {
    ...
    /**
     * Этот метод сортирует список целых чисел.
     * Сначала он преобразует список в максимальную кучу.
     * Затем он меняет местами первый и последний элементы, уменьшает размер кучи на один и
     * снова преобразует оставшуюся часть списка в кучу.
    ...
    /**
     * Этот метод преобразует поддерево с корнем в i-м элементе в максимальную кучу, если он не является кучей.
     * Здесь n - это размер кучи.
     * Если элемент i меньше своих дочерних элементов, происходит обмен местами с наибольшим
     * из дочерних элементов, и затем процесс повторяется для поддерева.
     *
     * @param list
     * @param n
     * @param i
